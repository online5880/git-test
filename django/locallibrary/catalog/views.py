...
from .serializer import BookInstanceSerialize, BookSerializer
...
class AllBorrowedBookListView(LoginRequiredMixin, PermissionRequiredMixin,
                              generic.ListView):
    model = BookInstance
    ...
    def get_queryset(self):
        ...
        return (BookInstance.objects.filter(borrower__isnull=False).filter(
            status__exact="o").order_by("due_back"))
...
# API 를 리턴하기 위해서는 APIView를 상속 받아야 한다.
...
class BookInstanceAPIView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            return Response({"error": "book_id must be an integer"},
                            status=400)

        copy_list = BookInstance.objects.filter(book__id=book_id)

        serialize = BookInstanceSerialize(copy_list, many=True)
